...
const tabs = [
    ...
    { value: "submissions", label: "Kaydettiklerin" },
]
...
const page = ({ params }: EditorPageProps) => {
  ...
  const handleChangeTab = (tab: typeof tabs[0]) => {
    // change last part of url
    router.push(`/problems/${params.slug}/${tab.value}`)
  }
    ...
    <div id='editor' className='h-full flex flex-col max-h-screen overflow-hidden'>
      ...
      <main className='h-full flex-1 bg-slate-100 dark:bg-slate-800 dark:text-foreground'>
        <Split
          ...
          minSize={[300, 400]}
        >
          <Tabs defaultValue={params.tab} className="h-full flex-col flex e-side-info border bg-background">
            <TabsList className='h-10 border-b bg-transparent justify-start rounded-none text-left'>
              {tabs.map((tab, i) => (
                  <TabsTrigger onClick={() => handleChangeTab(tab)} value={tab.value}>{tab.label}</TabsTrigger>
              ))}
            </TabsList>
